...
#  Reubicado en 'caso2/ec_seguro_salud/'. Se puede ejecutar desde cualquier wd
#  (raíz del proyecto, 'caso2/' o 'caso2/ec_seguro_salud/'): localiza el DGP por
#  ruta absoluta y ESCRIBE siempre dentro de esta carpeta (ec_seguro_salud/).
#  Los .rds conservan attr(.,"verdad"); los .csv van sin ella (trabajar a ciegas).
# =============================================================================

# Localiza R/dgp_conteos.R desde las ubicaciones habituales y deriva la carpeta caso2/.
.cand <- c("R/dgp_conteos.R", "caso2/R/dgp_conteos.R",
           "../R/dgp_conteos.R", "../caso2/R/dgp_conteos.R")
.dgp  <- .cand[file.exists(.cand)][1]
if (is.na(.dgp))
  stop("No encuentro 'R/dgp_conteos.R'. Abre GLM.Rproj, o fija el wd en 'caso2/' ",
       "o en 'caso2/ec_seguro_salud/'.")
.dgp      <- normalizePath(.dgp)
caso2_dir <- dirname(dirname(.dgp))         # .../caso2  (NO se cambia el wd)
source(.dgp)                                # dgp_conteos.R usa .raiz_glm(): independiente del wd

K_eq    <- 10L
out_dir <- file.path(caso2_dir, "ec_seguro_salud", "datos_equipos_salud")   # escribe aquí, siempre
